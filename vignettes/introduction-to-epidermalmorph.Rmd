...
For analysis with `epidermalmorph`, images need to be segmented (or traced). This can be done manually or with some degree of automation, but requires cell wall pixels to be black (value of 0). Other cell types can be labelled by using different values (colours). Here is what that might look like: 
...
```{r eval=FALSE}
polys1.1 <- image_to_poly(dir='.', file="traced_image.tif")
```
```{r echo=FALSE}
load("polys1-1.RData")
```
...
```
We can also check that the cell types have been preserved: 
```{r eval=FALSE}
...
traits1.1 <- extract_epidermal_traits(image.ID = "Image1.1", 
                                      ...
                                      image.scale = 2.954,  verbose=TRUE,) 
...
names <- c("Acmopyle_pancheri_1_1.tif",
           "Acmopyle_pancheri_1_2.tif",
           "Acmopyle_pancheri_1_3.tif",
           "Podocarpus_dispermus_1_1.tif",
           "Podocarpus_dispermus_1_2.tif",
           "Podocarpus_dispermus_1_3.tif",
           "Sundacarpus_amarus_1_1.tif",
           "Sundacarpus_amarus_1_2.tif",
           "Sundacarpus_amarus_1_3.tif")
...
For now, we just want to compare the overall measurements, so we'll bind the first element of each list together, and scale our trait measurements so that they can be be compared (I'm saving the global means and standard deviations to be used later): 
```{r}
